...
const Header = () => {
    ...
    const has = url.substring(url.indexOf('#'));
    useEffect(()=> {
        if(has.includes('#')){
            setNavSection(has)
        }
    ...
    return (
        <div className="w-full flex justify-center">
            ...
            <nav className={`fixed  z-[999] flex items-center gap-5 bg-slate-200/60 px-6 py-3 backdrop-blur-md rounded-full text-dark_primary duration-300 ${open ? ' bottom-10 ' : ' bottom-[-100%] '}"`}>
                <a href="#home" 
                    onClick={() => {
                        setNavSection('#home')
                    }}
                    className={`text-xl p-2.5 rounded-full sm:cursor-pointer ${navSection === '#home' ? 'bg-dark_primary text-white' : 'false'}`}>
                    <svg stroke="currentColor" fill="none" strokeWidth="2" viewBox="0 0 24 24" strokeLinecap="round" strokeLinejoin="round" height="1em" width="1em" xmlns="http://www.w3.org/2000/svg"><desc></desc><path stroke="none" d="M0 0h24v24H0z" fill="none"></path><path d="M19 8.71l-5.333 -4.148a2.666 2.666 0 0 0 -3.274 0l-5.334 4.148a2.665 2.665 0 0 0 -1.029 2.105v7.2a2 2 0 0 0 2 2h12a2 2 0 0 0 2 -2v-7.2c0 -.823 -.38 -1.6 -1.03 -2.105"></path><path d="M16 15c-2.21 1.333 -5.792 1.333 -8 0"></path></svg>
                </a>
                <a href="#skills"
                    onClick={() => {
                        setNavSection('#skills')
                    }}
                     className={`text-xl p-2.5 rounded-full sm:cursor-pointer ${navSection === '#skills' ? 'bg-dark_primary text-white' : 'false'}`}>
                ...
                <a href="#services"
                    onClick={() => {
                        setNavSection('#services')
                    }}
                className={`text-xl p-2.5 rounded-full sm:cursor-pointer ${navSection === '#services' ? 'bg-dark_primary text-white' : 'false'}`}>
                ...
                className={`text-xl p-2.5 rounded-full sm:cursor-pointer ${navSection === '#projects' ? 'bg-dark_primary text-white' : 'false'}`}>
                ...
                <a href="#contact"
                    ...
                    className={`text-xl p-2.5 rounded-full sm:cursor-pointer ${navSection === '#contact' ? 'bg-dark_primary text-white' : 'false'}`}>
